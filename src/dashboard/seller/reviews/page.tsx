...
import { getMockAllReviewsForNimali, getMockProductsByArtisanId } from '@/lib/mock-data';
...
export default function ViewReviewsPage() {
  ...
  const ratingFilterStaticOptions = [
  ...
  useEffect(() => {
    ...
    const fetchReviewsAndProducts = async () => {
      setIsLoading(true);
      // NOTE: Using mock function for Nimali, but could be adapted for currentUser.id
      const reviews = await getMockAllReviewsForNimali(); 
      setAllReviews(reviews);

      const sellerProducts: Product[] = await getMockProductsByArtisanId(currentUser.id);
      const pOptions = [
      ...
      setProductFilterOptions(pOptions);
      setIsLoading(false);
    };
        ...
        <CardHeader>
          ...
          <div className="pt-4 flex flex-col sm:flex-row gap-4">
            ...
            <Select value={ratingFilter} onValueChange={setRatingFilter}>
              ...
              <SelectContent>
                {ratingFilterStaticOptions.map(option => (
                  <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>
                ))}
              </SelectContent>
            </Select>
